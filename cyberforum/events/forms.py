...
class EventReportForm(forms.ModelForm):
    ...
    manual_title = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "id": "id_manual_title"}
        ),
    )
    ...
    class Meta:
        ...
        widgets = {
            "audience": forms.Select(
                attrs={"class": "form-select", "id": "id_audience"}
            ),
            "custom_audience": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "Например: студенты колледжей",
                }
            ),
            "listener_count": forms.NumberInput(
    ...
    def clean(self):
        ...
        if not existing_event and not manual_title:
            raise forms.ValidationError(
                "Укажите тему: выберите из списка или введите вручную."
            )
        ...
        if audience != "другое" and custom_audience:
            raise forms.ValidationError(
                "Поле «Своя аудитория» нужно заполнять только при выборе «Другое»."
            )
